...
### Decisiones cerradas (v0.7.1 — FB-ADJ-01, ajuste inter-fase) — 2026-08-03
**Renombre de etiqueta:** "Formularios" → "Ingreso" en el menú (sidebar + título de página); ruta (`/formularios`) y contenido "próximamente" sin cambios (§4) · **Admin envía Ausencia/Pasaje para sí:** admin-para-sí solamente (por otros queda fuera de alcance, decisión aparte) — sin migración: `ausencias_insert_admin`/`pasajes_insert_admin` ya permitían el INSERT propio sin restricción de `estado`, y ni `resolver_ausencia_request` ni `resolver_pasaje_request` tenían guarda de auto-aprobación; la Server Action crea la solicitud `pendiente` e invoca la RPC de resolución (`p_accion='aprobar'`) en el mismo request, con la sesión real del admin — si la resolución falla, borra la solicitud recién creada (sin huérfanas) (§4/§6.1/§7) · **excepción explícita a "nada se autoactiva":** admin-para-sí es la única excepción, junto a la ya existente de `uploadDocumentForEmployee` (CLAUDE.md, carga de documentos en nombre del empleado) (§7) · **diálogo de confirmación previo**, solo para admin, antes de enviar.
