...
test.describe('Empleado: Solicitud de Ausencia', () => {
  ...
  test('crea una solicitud de vacaciones (rango de fechas) y queda Pendiente', async ({ page }) => {
    ...
    await page.getByLabel(copy.solicitudAusencia.fields.motivo).selectOption('vacaciones');
    await page.getByLabel(copy.solicitudAusencia.fields.fechaInicio).fill(futureDate(60));
    await page.getByLabel(copy.solicitudAusencia.fields.fechaFin).fill(futureDate(62));
    await page.getByRole('button', { name: copy.solicitudAusencia.submitButton }).click();
...
test.describe('Empleado: Solicitud de Pasaje', () => {
  ...
  test('crea una solicitud con días de viaje discretos (no un rango) y queda Pendiente', async ({ page }) => {
    ...
    await page.getByLabel(copy.solicitudPasaje.fields.motivoViaje).selectOption('traslado_proyectos');
    await page.getByLabel(copy.solicitudPasaje.fields.origen).fill('Base Mendoza');
    await page.getByLabel(copy.solicitudPasaje.fields.destino).fill('Parque eólico Sitio 7');
    ...
    await page.getByLabel(`${copy.solicitudPasaje.fields.diasViaje} 1`).fill(futureDate(70));
    await page.getByRole('button', { name: copy.solicitudPasaje.diasViaje.agregarDia }).click();
    await page.getByLabel(`${copy.solicitudPasaje.fields.diasViaje} 2`).fill(futureDate(75));

    await page.getByRole('button', { name: copy.solicitudPasaje.submitButton }).click();
